...
function Image:size(width,height,filter)
   ...
   if width or height then
      -- Get filter:
      local filter = clib[('Undefined') .. 'Filter']
...
--added by luis
function Image:thumbnail4LargeImgs(width, height)
        local mwidth = width
        local mheight = height
        ...
        if mwidth and mheight then
                ...
                return {result = true, rwidth = cwidth, rheight = cheight, x = (width - cwidth) / 2, y =  (height - cheight) / 2}

        elseif mwidth then
                --only with the width
                local cwidth,cheight = self:size()
                --only when the width is bigger then the width limitation and the expect width then needs to generate thumbnail      
                if cwidth < mwidth * 1.1 then
                        return {result = false, rwidth = cwidth, rheiht = cheight, x = 0, y = 0}
                end

                self:size(mwidth, cheight * mwidth / cwidth)
                return {result = true, rwidth = mwidth, rheight = cheight * mwidth / cwidth, x = 0, y = 0} 
        else
                --only deliver the height
                local cwidth,cheight = self:size()
                --only when the height is bigger then the height limitation and the expect height then needs to generate thumbnail   
                if cheight < mheight * 1.1 then
                        return {result = false, rwidth = cwidth , rheight = cheight, x = 0, y = 0 }
                end

                self:size(cwidth *  mheight / cheight, mheight)
                return {result = true, rwidth = cwidth *  mheight / cheight, rheight = mheight, x =  0, y = 0}
        end

end


function Image:addWaterMark(waterMarkImagePath, drawOp, x, y, width, height )
  local image = Image.new(waterMarkImagePath)
  if image == nil then
	return nil
  end
  ...
  clib.MagickCompositeImage(self.wand,image.wand,clib[drawOp],x, y)
end
